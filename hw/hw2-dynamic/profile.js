...
elBtn.onclick = function () {
    ...
    if (errMsg.length === 0) {
        ...
        // First form the alert message about the fields to be updated.
        var updateMsg = "The following fields are to be updated: \n";
        updateMsg += updatedFields.map(function (field) {
            return (field.inputAreaID + ": from " + field.outputField.innerHTML 
                + " to " + field.inputField.value);
        }).join("\n");
        if (ifPasswordChanged()) {
            updateMsg += "\n" + "password (hidden for safty reason)";
        }
